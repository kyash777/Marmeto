...
productPriceElement.textContent = data.product.price; // Get vendor from data and display it
...
//   });
  ...
  sizeOptions.forEach(size => {
    ...
    radioInput.name = "size";
    radioInput.value = size;
    radioInput.id = `size-${size}`; // Assign a unique ID for the label
    ...
    const label = document.createElement("label");
    label.textContent = size;
    label.htmlFor = radioInput.id; // Link the label to the radio button
    radioDiv.appendChild(label);
